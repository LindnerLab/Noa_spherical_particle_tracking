...
U = 6.89E-02; %m/sec % !!!!!!! Adjust according to experiment
...
file_folder = 'C:\Users\labo\Desktop\Noa\water-glycerol_80um_190620\5ml_min_3000fps_24C\particles_coordinates';    %  Enter name of folder from which you want to upload pictures with full path
folder = pwd;
        ...
        for k = 1 : length(track_points)-1    % length of each trajectory
         ...
         for k = 1 : length(track_points)-2
             ...
             if   vAvg ~= 0 % this is to avoid data that is 0
                 if vAvg < 2 %vMax/3 %to avoid extremely high velocity values determined by a single vMax value
                     idx = ceil((vAvg/(2)*numColors)); % look up color for current velocity normalize by maximum velocity  %    idx = ceil(vAvg/vMax*numColors); % normalize by maximum velocity
                     color = cmap(idx,:);
