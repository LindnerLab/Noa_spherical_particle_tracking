...
image_folder = 'C:\Users\labo\Desktop\Noa\water-glycerol_80um_190620\5ml_min_3000fps_24C\raw_images';    %  Enter name of folder from which you want to upload pictures 
verbose = true;                                         % If verbose is true, the script will create and save a figure of the particle finding
r = 5;                                                 % Expected radius of the particles in pixels
...
Img = zeros(Img_size(1)+60,Img_size(2)+60, total_images);     % Pre-allocate memory for the cropped images (is faster than allocating space when needed.
...
for n = 1:total_images
    ...
    Img(:,:,n) = padarray(Img_temp,[30 30],0,'both');        % Import, crop, convert the image to double and invert it
end
...
%Img_bg = histeq(Img_bg);

% pass images through a filter, find circles and save x,y coordinates to *.txt files
for n = 1:total_images    
    % Find circles
    Img_bpass = bpass(Img_bg(:,:,n),1,40);            % filter - smoothing and subtracting the background (picture, 1, need to adjust to fit with particle diameter)
    Img_bpass = Img_bpass - 20;
    Img_bpass(Img_bpass < 0) = 0;
    
  %  [allcenters1] = FindParticlesConvolution(Img_bpass,r,SelectionCriteria, threshold, false);
    [centers1, radii1, metric1] = imfindcircles(Img_bpass,[5,15]); %finds the particles with defined centers - the particles in the focus plane. [ radii range ]
